...
public class PolylineEncoder {
    ...
    public HashMap<String, String> dpEncode(Track track) {
        ...
        if (points.size() > 2) {
            ...
            while (stack.size() > 0) {
                ...
                segmentLength =
                    Math.pow(points.get(current[1]).getLatDouble() - points.get(current[0]).getLatDouble(), 2) +
    ...
    public double distance(Trackpoint p0, Trackpoint p1, Trackpoint p2, double segLength) {
        ...
        if (p1_lat == p2_lat && p1_lon == p2_lon) {
            ...
            if (0 < u && u < 1) {
                out = Math.sqrt(Math.pow(p0_lat - p1_lat - u * (p2_lat - p1_lat), 2) +
                                Math.pow(p0_lon - p1_lon - u * (p2_lon - p1_lon), 2));
            }
        }
        return out;
    }
    ...
    /**
     * @param points set the points that should be encoded all points have to be in
    ...
    /**
     * @param LineString set the points that should be encoded all points have to be in
